...
''' </summary>
''' <remarks></remarks>
Public Class LNM0014ReutrmHistory
    Inherits Page
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As MySqlConnection)
        ...
        '○ 検索SQL
        '　検索説明
        '     条件指定に従い該当データを通運発送料履歴テーブルから取得する
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(ARRTRUSTEECD), '')                                    AS ARRTRUSTEECD     ")
        SQLStr.AppendLine("   , coalesce(RTRIM(ARRTRUSTEESUBCD), '')                                 AS ARRTRUSTEESUBCD  ")
        SQLStr.AppendLine("   , coalesce(FORMAT(SPRCURSTYMD, 'yyyy/MM/dd'), '')                      AS SPRCURSTYMD      ")
        SQLStr.AppendLine("   , coalesce(FORMAT(SPRCURENDYMD, 'yyyy/MM/dd'), '')                     AS SPRCURENDYMD     ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRCURSHIPFEE), '')                                   AS SPRCURSHIPFEE    ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRCURARRIVEFEE), '')                                 AS SPRCURARRIVEFEE  ")
        ...
        SQLStr.AppendLine("   , coalesce(FORMAT(SPRNEXTSTYMD, 'yyyy/MM/dd'), '')                     AS SPRNEXTSTYMD     ")
        SQLStr.AppendLine("   , coalesce(FORMAT(SPRNEXTENDYMD, 'yyyy/MM/dd'), '')                    AS SPRNEXTENDYMD    ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRNEXTSHIPFEE), '')                                  AS SPRNEXTSHIPFEE   ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRNEXTARRIVEFEE), '')                                AS SPRNEXTARRIVEFEE ")
        ...
        SQLStr.AppendLine("     LNG.LNT0117_REUTRHIST                                                                             ")
    ...
    Protected Sub MODIFYYMGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine(" FROM lng.LNT0117_REUTRHIST ")
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0117_REUTRHIST SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNT0117_REUTRHIST Select"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                             'ログ出力
...
#Region "ﾀﾞｳﾝﾛｰﾄﾞ"
    ...
    Protected Sub WF_EXCELPDF(ByVal WW_FILETYPE As Integer)
        ...
        wb.ActiveSheet.Range("C1").Value = "通運発送料変更履歴一覧"
    ...
    Public Sub SetHEADER(ByVal sheet As IWorksheet, ByVal WW_MAXCOL As Integer)
        ...
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.BIGCTNCD).Value = "大分類コード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.MIDDLECTNCD).Value = "中分類コード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = "発駅コード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = "発受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBCD).Value = "発受託人サブコード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.PRIORITYNO).Value = "優先順位"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.PURPOSE).Value = "使用目的"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.ARRSTATION).Value = "着駅コード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.ARRTRUSTEECD).Value = "着受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.ARRTRUSTEESUBCD).Value = "着受託人サブコード"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURSTYMD).Value = "特例置換項目-現行開始適用日"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURENDYMD).Value = "特例置換項目-現行終了適用日"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURSHIPFEE).Value = "特例置換項目-現行発送料"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURARRIVEFEE).Value = "特例置換項目-現行到着料"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURROUNDKBN).Value = "特例置換項目-現行端数処理区分"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTSTYMD).Value = "特例置換項目-次期開始適用日"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTENDYMD).Value = "特例置換項目-次期終了適用日"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTSHIPFEE).Value = "特例置換項目-次期発送料"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTARRIVEFEE).Value = "特例置換項目-次期到着料"
        sheet.Cells(WW_HEADERROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTROUNDKBN).Value = "特例置換項目-次期端数処理区分"
    ...
    Public Sub SetDETAIL(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        For Each Row As DataRow In LNM0014tbl.Rows
            ...
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.BIGCTNCD).Value = Row("BIGCTNCD") '大分類コード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.MIDDLECTNCD).Value = Row("MIDDLECTNCD") '中分類コード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = Row("DEPSTATION") '発駅コード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = Row("DEPTRUSTEECD") '発受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBCD).Value = Row("DEPTRUSTEESUBCD") '発受託人サブコード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.PRIORITYNO).Value = Row("PRIORITYNO") '優先順位
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.PURPOSE).Value = Row("PURPOSE") '使用目的
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.ARRSTATION).Value = Row("ARRSTATION") '着駅コード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.ARRTRUSTEECD).Value = Row("ARRTRUSTEECD") '着受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.ARRTRUSTEESUBCD).Value = Row("ARRTRUSTEESUBCD") '着受託人サブコード
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURSTYMD).Value = Row("SPRCURSTYMD") '特例置換項目-現行開始適用日
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURENDYMD).Value = Row("SPRCURENDYMD") '特例置換項目-現行終了適用日
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURSHIPFEE).Value = Row("SPRCURSHIPFEE") '特例置換項目-現行発送料
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURARRIVEFEE).Value = Row("SPRCURARRIVEFEE") '特例置換項目-現行到着料
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRCURROUNDKBN).Value = Row("SPRCURROUNDKBN") '特例置換項目-現行端数処理区分
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTSTYMD).Value = Row("SPRNEXTSTYMD") '特例置換項目-次期開始適用日
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTENDYMD).Value = Row("SPRNEXTENDYMD") '特例置換項目-次期終了適用日
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTSHIPFEE).Value = Row("SPRNEXTSHIPFEE") '特例置換項目-次期発送料
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTARRIVEFEE).Value = Row("SPRNEXTARRIVEFEE") '特例置換項目-次期到着料
            sheet.Cells(WW_ACTIVEROW, LNM0014WRKINC.HISTORYEXCELCOL.SPRNEXTROUNDKBN).Value = Row("SPRNEXTROUNDKBN") '特例置換項目-次期端数処理区分
