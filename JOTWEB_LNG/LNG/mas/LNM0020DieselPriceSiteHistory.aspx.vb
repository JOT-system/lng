...
''' </summary>
''' <remarks></remarks>
Public Class LNM0020DieselPriceSiteHistory
    Inherits Page
                ...
                If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                    ...
                    Select Case WF_ButtonClick.Value
                        ...
                        Case "WF_ButtonPRINT"           '一覧印刷ボタン押下
                            WF_EXCELPDF(LNM0020WRKINC.FILETYPE.PDF)
                        Case "WF_ButtonEND", "LNM0020L"  '戻るボタン押下（LNM0020Lは、パンくずより）
                            WF_ButtonEND_Click()
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = LNM0020WRKINC.MAPIDH
        '○ HELP表示有無設定
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine("      WHEN COALESCE(RTRIM(LNT33.OPERATEKBN), '') ='2' AND COALESCE(RTRIM(LNT33.MODIFYKBN), '') ='2' THEN '変更前 更新' ")
        SQLStr.AppendLine("      WHEN COALESCE(RTRIM(LNT33.OPERATEKBN), '') ='2' AND COALESCE(RTRIM(LNT33.MODIFYKBN), '') ='3' THEN '変更後 更新' ")
        SQLStr.AppendLine("      WHEN COALESCE(RTRIM(LNT33.OPERATEKBN), '') ='3' AND COALESCE(RTRIM(LNT33.MODIFYKBN), '') ='2' THEN '変更前 削除' ")
        SQLStr.AppendLine("      WHEN COALESCE(RTRIM(LNT33.OPERATEKBN), '') ='3' AND COALESCE(RTRIM(LNT33.MODIFYKBN), '') ='3' THEN '変更後 削除' ")
        SQLStr.AppendLine("      ELSE ''                                                                 ")
        SQLStr.AppendLine("    END AS OPERATEKBNNAME                                                     ")
        SQLStr.AppendLine("   , COALESCE(RTRIM(LNT33.MODIFYKBN), '')            AS MODIFYKBN             ")
        SQLStr.AppendLine("   ,CASE COALESCE(RTRIM(LNT33.MODIFYKBN), '')                                 ")
        ...
        SQLStr.AppendLine("   , DATE_FORMAT(LNT33.MODIFYYMD, '%Y/%m/%d %T')     AS MODIFYYMD             ")
        SQLStr.AppendLine("   , COALESCE(RTRIM(LNT33.MODIFYUSER), '')           AS MODIFYUSER            ")
        SQLStr.AppendLine(" FROM                                                                         ")
        SQLStr.AppendLine("     LNG.LNT0033_DIESELPRICESITEHIST LNT33                                    ")
        SQLStr.AppendLine(" WHERE                                                                        ")
        '変更日が指定されている場合
        If Not WF_DDL_MODIFYDD.SelectedValue = "" Then
            SQLStr.AppendLine("    DATE_FORMAT(LNT33.MODIFYYMD,'%Y/%m/%d')  = @MODIFYYMD                 ")
        Else
            SQLStr.AppendLine("    DATE_FORMAT(LNT33.MODIFYYMD,'%Y/%m/01')  = @MODIFYYMD                 ")
        End If
        '変更ユーザが指定されている場合
        If Not WF_DDL_MODIFYUSER.SelectedValue = "" Then
            SQLStr.AppendLine(" AND COALESCE(RTRIM(LNT33.MODIFYUSER), '')  =  @MODIFYUSER                ")
        End If
        ...
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0033_DIESELPRICESITEHIST SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNT0033_DIESELPRICESITEHIST Select"
    ...
    Protected Sub MODIFYYMGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine(" FROM LNG.LNT0033_DIESELPRICESITEHIST ")
    ...
    Protected Sub DisplayGrid()
        ...
        '○ 表示対象行カウント(絞り込み対象)
        For Each LNM0020row As DataRow In LNM0020tbl.Rows
            If LNM0020row("HIDDEN") = 0 Then
                WW_DataCNT += 1
                ' 行(LINECNT)を再設定する。既存項目(SELECT)を利用
                LNM0020row("SELECT") = WW_DataCNT
            End If
        Next
    ...
    Protected Sub WF_ButtonEND_Click()
        'パンくずから検索を選択した場合
        If WF_ButtonClick.Value = "LNM0020S" Then
...
#Region "ﾀﾞｳﾝﾛｰﾄﾞ"
    ...
    Protected Sub WF_EXCELPDF(ByVal WW_FILETYPE As Integer)
        ...
        WW_MAXCOL = [Enum].GetValues(GetType(LNM0020WRKINC.HISTORYEXCELCOL)).Cast(Of Integer)().Max()
        ...
        wb.ActiveSheet.Range("C1").Value = "軽油価格参照先マスタ変更履歴一覧"
    ...
    Public Sub SetHEADER(ByVal sheet As IWorksheet, ByVal WW_MAXCOL As Integer)
        ...
        sheet.Cells(WW_HEADERROW, LNM0020WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = "操作区分"
        sheet.Cells(WW_HEADERROW, LNM0020WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = "変更区分"
        sheet.Cells(WW_HEADERROW, LNM0020WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = "変更日時"
    ...
    Public Sub SetDETAIL(ByVal wb As Workbook, ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        For Each Row As DataRow In LNM0020tbl.Rows
            '値
            sheet.Cells(WW_ACTIVEROW, LNM0020WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = Row("OPERATEKBNNAME") '操作区分
            sheet.Cells(WW_ACTIVEROW, LNM0020WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = Row("MODIFYKBNNAME") '変更区分
            sheet.Cells(WW_ACTIVEROW, LNM0020WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = Row("MODIFYYMD") '変更日時
    ...
    Public Sub SetMODIFYHATCHING(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        WW_MAXCOL = [Enum].GetValues(GetType(LNM0020WRKINC.HISTORYEXCELCOL)).Cast(Of Integer)().Max()
        WW_STCOL = LNM0020WRKINC.HISTORYEXCELCOL.DELFLG   '削除フラグ
