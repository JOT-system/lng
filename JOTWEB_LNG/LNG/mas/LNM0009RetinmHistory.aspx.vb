...
''' </summary>
''' <remarks></remarks>
Public Class LNM0009RetinmHistory
    Inherits Page
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = LNM0009WRKINC.MAPIDH
        '○ HELP表示有無設定
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(KIRO), '')                                                      AS KIRO                     ")
        SQLStr.AppendLine("   , coalesce(RTRIM(TUNRENTRATE), '')                                               AS TUNRENTRATE              ")
        SQLStr.AppendLine("   , coalesce(FORMAT(NEXTFROMYMD, 'yyyy/MM/dd'), '')                                AS NEXTFROMYMD              ")
        SQLStr.AppendLine("   , coalesce(RTRIM(NEXTTUNRENTRATE), '')                                           AS NEXTTUNRENTRATE          ")
        ...
        SQLStr.AppendLine("     LNG.LNT0118_RETINHIST                                                                                     ")
        ...
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNM0009H SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNM0009H Select"
    ...
    Protected Sub MODIFYYMGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine(" FROM lng.LNT0118_RETINHIST ")
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0118_RETINHIST SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNT0118_RETINHIST Select"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                             'ログ出力
...
#Region "ﾀﾞｳﾝﾛｰﾄﾞ"
    ...
    Protected Sub WF_EXCELPDF(ByVal WW_FILETYPE As Integer)
        ...
        '最大列(RANGE)を取得
        Dim WW_MAXCOL As Integer = [Enum].GetValues(GetType(LNM0009WRKINC.HISTORYEXCELCOL)).Cast(Of Integer)().Max()
        ...
        wb.ActiveSheet.Range("C1").Value = "ＪＲ賃率変更履歴一覧"
    ...
    Public Sub SetHEADER(ByVal sheet As IWorksheet, ByVal WW_MAXCOL As Integer)
        ...
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = "操作区分"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = "変更区分"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = "変更日時"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.MODIFYUSER).Value = "変更USER"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.DELFLG).Value = "削除フラグ"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.KIRO).Value = "キロ程"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.TUNRENTRATE).Value = "現行１屯当りの賃率"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.NEXTFROMYMD).Value = "次期適用年月日"
        sheet.Cells(WW_HEADERROW, LNM0009WRKINC.HISTORYEXCELCOL.NEXTTUNRENTRATE).Value = "次期１屯当りの賃率"
    ...
    Public Sub SetDETAIL(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        For Each Row As DataRow In LNM0009tbl.Rows
            '値
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = Row("OPERATEKBNNAME") '操作区分	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = Row("MODIFYKBNNAME") '変更区分	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = Row("MODIFYYMD") '変更日時	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.MODIFYUSER).Value = Row("MODIFYUSER") '変更USER	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.DELFLG).Value = Row("DELFLG") '削除フラグ	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.KIRO).Value = Row("KIRO") 'キロ程	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.TUNRENTRATE).Value = Row("TUNRENTRATE") '現行１屯当りの賃率	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.NEXTFROMYMD).Value = Row("NEXTFROMYMD") '次期適用年月日	
            sheet.Cells(WW_ACTIVEROW, LNM0009WRKINC.HISTORYEXCELCOL.NEXTTUNRENTRATE).Value = Row("NEXTTUNRENTRATE") '次期１屯当りの賃率	

            '変更区分が変更後の行の場合
            If Row("MODIFYKBN") = LNM0009WRKINC.MODIFYKBN.AFTDATA Then
                '変更箇所を塗りつぶし
    ...
    Public Sub SetMODIFYHATCHING(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        '変更チェック開始列を取得
        Dim WW_STCOL As Integer = LNM0009WRKINC.HISTORYEXCELCOL.DELFLG   '削除フラグ
