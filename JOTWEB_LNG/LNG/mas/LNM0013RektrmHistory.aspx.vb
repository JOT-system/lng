...
''' </summary>
''' <remarks></remarks>
Public Class LNM0013RektrmHistory
    Inherits Page
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = LNM0013WRKINC.MAPIDH
        '○ HELP表示有無設定
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As MySqlConnection)
        ...
        '○ 検索SQL
        '　検索説明
        '     条件指定に従い該当データを回送運賃適用率変更履歴テーブルから取得する
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(JRDEPBRANCHCD), '')                                   AS JRDEPBRANCHCD    ")
        SQLStr.AppendLine("   , coalesce(RTRIM(ARRSTATION), '')                                      AS ARRSTATION       ")
        SQLStr.AppendLine("   , coalesce(RTRIM(JRARRBRANCHCD), '')                                   AS JRARRBRANCHCD    ")
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRCURSTYMD), '')                                     AS SPRCURSTYMD      ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRCURENDYMD), '')                                    AS SPRCURENDYMD     ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRCURAPPLYRATE), '')                                 AS SPRCURAPPLYRATE  ")
        SQLStr.AppendLine("   , NULLIF(RTRIM(SPRCURROUNDKBN), 0)                                   AS SPRCURROUNDKBN   ")
        SQLStr.AppendLine("   , SUBSTRING(coalesce(NULLIF(RTRIM(SPRCURROUNDKBN), 0), ''),1,1)        AS SPRCURROUNDKBN1  ")
        SQLStr.AppendLine("   , SUBSTRING(coalesce(NULLIF(RTRIM(SPRCURROUNDKBN), 0), ''),2,1)        AS SPRCURROUNDKBN2  ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRNEXTSTYMD), '')                                    AS SPRNEXTSTYMD     ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRNEXTENDYMD), '')                                   AS SPRNEXTENDYMD    ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SPRNEXTAPPLYRATE), '')                                AS SPRNEXTAPPLYRATE ")
        SQLStr.AppendLine("   , NULLIF(RTRIM(SPRNEXTROUNDKBN), 0)                                  AS SPRNEXTROUNDKBN  ")
        SQLStr.AppendLine("   , SUBSTRING(coalesce(NULLIF(RTRIM(SPRNEXTROUNDKBN), 0), ''),1,1)       AS SPRNEXTROUNDKBN1 ")
        SQLStr.AppendLine("   , SUBSTRING(coalesce(NULLIF(RTRIM(SPRNEXTROUNDKBN), 0), ''),2,1)       AS SPRNEXTROUNDKBN2 ")
        ...
        SQLStr.AppendLine("     LNG.LNT0116_REKTRHIST                                                                             ")
        ...
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNM0013H SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNM0013H Select"
    ...
    Protected Sub MODIFYYMGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine(" FROM lng.LNT0116_REKTRHIST ")
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0116_REKTRHIST SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNT0116_REKTRHIST Select"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                             'ログ出力
...
#Region "ﾀﾞｳﾝﾛｰﾄﾞ"
    ...
    Protected Sub WF_EXCELPDF(ByVal WW_FILETYPE As Integer)
        ...
        '最大列(RANGE)を取得
        Dim WW_MAXCOL As Integer = [Enum].GetValues(GetType(LNM0013WRKINC.HISTORYEXCELCOL)).Cast(Of Integer)().Max()
        ...
        wb.ActiveSheet.Range("C1").Value = "回送運賃適用率変更履歴一覧"
    ...
    Public Sub SetHEADER(ByVal sheet As IWorksheet, ByVal WW_MAXCOL As Integer)
        ...
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = "操作区分"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = "変更区分"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = "変更日時"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.MODIFYUSER).Value = "変更USER"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.DELFLG).Value = "削除フラグ"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.BIGCTNCD).Value = "大分類コード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.MIDDLECTNCD).Value = "中分類コード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.PRIORITYNO).Value = "優先順位"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = "発駅コード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.JRDEPBRANCHCD).Value = "ＪＲ発支社支店コード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.ARRSTATION).Value = "着駅コード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.JRARRBRANCHCD).Value = "ＪＲ着支社支店コード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.PURPOSE).Value = "使用目的"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = "発受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBCD).Value = "発受託人サブコード"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.CTNTYPE).Value = "コンテナ記号"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.CTNSTNO).Value = "コンテナ番号（開始）"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.CTNENDNO).Value = "コンテナ番号（終了）"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURSTYMD).Value = "特例置換項目-現行開始適用日"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURENDYMD).Value = "特例置換項目-現行終了摘要日"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURAPPLYRATE).Value = "特例置換項目-現行摘要率"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURROUNDKBN).Value = "特例置換項目-現行端数処理区分"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTSTYMD).Value = "特例置換項目-次期開始適用日"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTENDYMD).Value = "特例置換項目-次期終了摘要日"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTAPPLYRATE).Value = "特例置換項目-次期摘要率"
        sheet.Cells(WW_HEADERROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTROUNDKBN).Value = "特例置換項目-次期端数処理区分"
    ...
    Public Sub SetDETAIL(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        For Each Row As DataRow In LNM0013tbl.Rows
            '値
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = Row("OPERATEKBNNAME") '操作区分
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = Row("MODIFYKBNNAME") '変更区分
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = Row("MODIFYYMD") '変更日時
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.MODIFYUSER).Value = Row("MODIFYUSER") '変更USER
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.DELFLG).Value = Row("DELFLG") '削除フラグ
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.BIGCTNCD).Value = Row("BIGCTNCD") '大分類コード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.MIDDLECTNCD).Value = Row("MIDDLECTNCD") '中分類コード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.PRIORITYNO).Value = Row("PRIORITYNO") '優先順位
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = Row("DEPSTATION") '発駅コード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.JRDEPBRANCHCD).Value = Row("JRDEPBRANCHCD") 'ＪＲ発支社支店コード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.ARRSTATION).Value = Row("ARRSTATION") '着駅コード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.JRARRBRANCHCD).Value = Row("JRARRBRANCHCD") 'ＪＲ着支社支店コード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.PURPOSE).Value = Row("PURPOSE") '使用目的
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = Row("DEPTRUSTEECD") '発受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBCD).Value = Row("DEPTRUSTEESUBCD") '発受託人サブコード
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.CTNTYPE).Value = Row("CTNTYPE") 'コンテナ記号
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.CTNSTNO).Value = Row("CTNSTNO") 'コンテナ番号（開始）
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.CTNENDNO).Value = Row("CTNENDNO") 'コンテナ番号（終了）
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURSTYMD).Value = Row("SPRCURSTYMD") '特例置換項目-現行開始適用日
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURENDYMD).Value = Row("SPRCURENDYMD") '特例置換項目-現行終了摘要日
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURAPPLYRATE).Value = Row("SPRCURAPPLYRATE") '特例置換項目-現行摘要率
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRCURROUNDKBN).Value = Row("SPRCURROUNDKBN") '特例置換項目-現行端数処理区分
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTSTYMD).Value = Row("SPRNEXTSTYMD") '特例置換項目-次期開始適用日
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTENDYMD).Value = Row("SPRNEXTENDYMD") '特例置換項目-次期終了摘要日
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTAPPLYRATE).Value = Row("SPRNEXTAPPLYRATE") '特例置換項目-次期摘要率
            sheet.Cells(WW_ACTIVEROW, LNM0013WRKINC.HISTORYEXCELCOL.SPRNEXTROUNDKBN).Value = Row("SPRNEXTROUNDKBN") '特例置換項目-次期端数処理区分

            '変更区分が変更後の行の場合
            If Row("MODIFYKBN") = LNM0013WRKINC.MODIFYKBN.AFTDATA Then
                '変更箇所を塗りつぶし
    ...
    Public Sub SetMODIFYHATCHING(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        '変更チェック開始列を取得
        Dim WW_STCOL As Integer = LNM0013WRKINC.HISTORYEXCELCOL.DELFLG   '削除フラグ
