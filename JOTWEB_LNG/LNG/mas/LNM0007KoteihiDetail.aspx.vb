...
#End Region
    ...
    Protected Sub WF_SelectFIELD_CHANGE(ByVal resVal As String)
        ...
        If selectTORI <> "" AndAlso retKASANOrgList.Items.Count = 0 Then
            selectKASANORG = ""         '-- 加算先部門(表示)初期化
            selectindexKASANORG = 0     '-- 加算先部門(INDEX)初期化
            retKASANOrgList = LNM0007WRKINC.getDowpDownKasanOrgList(Master.MAPID, Master.ROLE_ORG, I_TORICODE:=selectTORI, I_ORGCODE:=WF_ORG.SelectedValue, I_KASANORGCODE:=selectKASANORG, I_CREATEFLG:=True)
        End If
