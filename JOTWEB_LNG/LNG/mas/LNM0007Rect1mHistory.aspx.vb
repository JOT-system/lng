...
''' </summary>
''' <remarks></remarks>
Public Class LNM0007Rect1mHistory
    Inherits Page
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As MySqlConnection)
        ...
        '○ 検索SQL
        '　検索説明
        '     条件指定に従い該当データをコード変換特例１変更履歴テーブルから取得する
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(CTNSTNO), '')                                   AS CTNSTNO                  ")
        SQLStr.AppendLine("   , coalesce(RTRIM(CTNENDNO), '')                                  AS CTNENDNO                 ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SLCSTACKFREEKBN), '')                           AS SLCSTACKFREEKBN          ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SLCSTATUSKBN), '')                              AS SLCSTATUSKBN             ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SLCDEPTRUSTEESUBCD), '')                        AS SLCDEPTRUSTEESUBCD       ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SLCDEPSHIPPERCD), '')                           AS SLCDEPSHIPPERCD          ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SLCARRSTATION), '')                             AS SLCARRSTATION            ")
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(SLCJRITEMCD), '')                               AS SLCJRITEMCD              ")
        SQLStr.AppendLine("   , coalesce(RTRIM(SLCPICKUPTEL), '')                              AS SLCPICKUPTEL             ")
        ...
        SQLStr.AppendLine("     LNG.LNT0112_RECT1HIST                                                                             ")
    ...
    Protected Sub MODIFYYMGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine(" FROM lng.LNT0112_RECT1HIST ")
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0112_RECT1HIST SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNT0112_RECT1HIST Select"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                             'ログ出力
...
#Region "ﾀﾞｳﾝﾛｰﾄﾞ"
    ...
    Protected Sub WF_EXCELPDF(ByVal WW_FILETYPE As Integer)
        ...
        '最大列(RANGE)を取得
        Dim WW_MAXCOL As Integer = [Enum].GetValues(GetType(LNM0007WRKINC.HISTORYEXCELCOL)).Cast(Of Integer)().Max()
        ...
        wb.ActiveSheet.Range("C1").Value = "コード変換特例１変更履歴一覧"
    ...
    Public Sub SetHEADER(ByVal sheet As IWorksheet, ByVal WW_MAXCOL As Integer)
        ...
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.ORGCODE).Value = "組織コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.BIGCTNCD).Value = "大分類コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.MIDDLECTNCD).Value = "中分類コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = "発駅コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = "発受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.PRIORITYNO).Value = "優先順位"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.PURPOSE).Value = "使用目的"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SMALLCTNCD).Value = "選択比較項目-小分類コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.CTNTYPE).Value = "選択比較項目-コンテナ記号"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.CTNSTNO).Value = "選択比較項目-コンテナ番号（開始）"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.CTNENDNO).Value = "選択比較項目-コンテナ番号（終了）"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCSTACKFREEKBN).Value = "選択比較項目-積空区分"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCSTATUSKBN).Value = "選択比較項目-状態区分"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCDEPTRUSTEESUBCD).Value = "選択比較項目-発受託人サブコード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCDEPSHIPPERCD).Value = "選択比較項目-発荷主コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCARRSTATION).Value = "選択比較項目-着駅コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCARRTRUSTEECD).Value = "選択比較項目-着受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCARRTRUSTEESUBCD).Value = "選択比較項目-着受託人サブコード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCJRITEMCD).Value = "選択比較項目-ＪＲ品目コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCPICKUPTEL).Value = "選択比較項目-集荷先電話番号"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPTRUSTEECD).Value = "特例置換項目-発受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPTRUSTEESUBCD).Value = "特例置換項目-発受託人サブコード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPTRUSTEESUBZKBN).Value = "特例置換項目-発受託人サブゼロ変換区分"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPSHIPPERCD).Value = "特例置換項目-発荷主コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRARRTRUSTEECD).Value = "特例置換項目-着受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRARRTRUSTEESUBCD).Value = "特例置換項目-着受託人サブ"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRARRTRUSTEESUBZKBN).Value = "特例置換項目-着受託人サブゼロ変換区分"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRJRITEMCD).Value = "特例置換項目-ＪＲ品目コード"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRSTACKFREEKBN).Value = "特例置換項目-積空区分"
        sheet.Cells(WW_HEADERROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRSTATUSKBN).Value = "特例置換項目-状態区分"
    ...
    Public Sub SetDETAIL(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        For Each Row As DataRow In LNM0007tbl.Rows
            ...
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.ORGCODE).Value = Row("ORGCODE") '組織コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.BIGCTNCD).Value = Row("BIGCTNCD") '大分類コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.MIDDLECTNCD).Value = Row("MIDDLECTNCD") '中分類コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = Row("DEPSTATION") '発駅コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = Row("DEPTRUSTEECD") '発受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.PRIORITYNO).Value = Row("PRIORITYNO") '優先順位
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.PURPOSE).Value = Row("PURPOSE") '使用目的
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SMALLCTNCD).Value = Row("SMALLCTNCD") '選択比較項目-小分類コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.CTNTYPE).Value = Row("CTNTYPE") '選択比較項目-コンテナ記号
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.CTNSTNO).Value = Row("CTNSTNO") '選択比較項目-コンテナ番号（開始）
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.CTNENDNO).Value = Row("CTNENDNO") '選択比較項目-コンテナ番号（終了）
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCSTACKFREEKBN).Value = Row("SLCSTACKFREEKBN") '選択比較項目-積空区分
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCSTATUSKBN).Value = Row("SLCSTATUSKBN") '選択比較項目-状態区分
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCDEPTRUSTEESUBCD).Value = Row("SLCDEPTRUSTEESUBCD") '選択比較項目-発受託人サブコード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCDEPSHIPPERCD).Value = Row("SLCDEPSHIPPERCD") '選択比較項目-発荷主コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCARRSTATION).Value = Row("SLCARRSTATION") '選択比較項目-着駅コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCARRTRUSTEECD).Value = Row("SLCARRTRUSTEECD") '選択比較項目-着受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCARRTRUSTEESUBCD).Value = Row("SLCARRTRUSTEESUBCD") '選択比較項目-着受託人サブコード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCJRITEMCD).Value = Row("SLCJRITEMCD") '選択比較項目-ＪＲ品目コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SLCPICKUPTEL).Value = Row("SLCPICKUPTEL") '選択比較項目-集荷先電話番号
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPTRUSTEECD).Value = Row("SPRDEPTRUSTEECD") '特例置換項目-発受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPTRUSTEESUBCD).Value = Row("SPRDEPTRUSTEESUBCD") '特例置換項目-発受託人サブコード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPTRUSTEESUBZKBN).Value = Row("SPRDEPTRUSTEESUBZKBN") '特例置換項目-発受託人サブゼロ変換区分
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRDEPSHIPPERCD).Value = Row("SPRDEPSHIPPERCD") '特例置換項目-発荷主コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRARRTRUSTEECD).Value = Row("SPRARRTRUSTEECD") '特例置換項目-着受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRARRTRUSTEESUBCD).Value = Row("SPRARRTRUSTEESUBCD") '特例置換項目-着受託人サブ
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRARRTRUSTEESUBZKBN).Value = Row("SPRARRTRUSTEESUBZKBN") '特例置換項目-着受託人サブゼロ変換区分
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRJRITEMCD).Value = Row("SPRJRITEMCD") '特例置換項目-ＪＲ品目コード
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRSTACKFREEKBN).Value = Row("SPRSTACKFREEKBN") '特例置換項目-積空区分
            sheet.Cells(WW_ACTIVEROW, LNM0007WRKINC.HISTORYEXCELCOL.SPRSTATUSKBN).Value = Row("SPRSTATUSKBN") '特例置換項目-状態区分
    ...
    Public Sub SetMODIFYHATCHING(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        '変更チェック開始列を取得
        Dim WW_STCOL As Integer = LNM0007WRKINC.HISTORYEXCELCOL.DELFLG   '削除フラグ
