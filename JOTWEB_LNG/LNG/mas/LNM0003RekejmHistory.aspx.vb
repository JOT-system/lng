...
''' </summary>
''' <remarks></remarks>
Public Class LNM0003RekejmHistory
    Inherits Page
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = LNM0003WRKINC.MAPIDH
        '○ HELP表示有無設定
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As MySqlConnection)
        ...
        '○ 検索SQL
        '　検索説明
        '     条件指定に従い該当データをコンテナ決済マスタ変更履歴テーブルから取得する
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(DEPTRUSTEENM), '')                      AS DEPTRUSTEENM         ")
        SQLStr.AppendLine("   , coalesce(RTRIM(DEPTRUSTEESUBNM), '')                   AS DEPTRUSTEESUBNM      ")
        SQLStr.AppendLine("   , coalesce(RTRIM(DEPTRUSTEESUBKANA), '')                 AS DEPTRUSTEESUBKANA    ")
        SQLStr.AppendLine("   , coalesce(RTRIM(TORICODE), '')                          AS TORICODE             ")
        SQLStr.AppendLine("   , coalesce(RTRIM(ELIGIBLEINVOICENUMBER), '')             AS ELIGIBLEINVOICENUMBER ")
        SQLStr.AppendLine("   , coalesce(RTRIM(INVKEIJYOBRANCHCD), '')                 AS INVKEIJYOBRANCHCD    ")
        SQLStr.AppendLine("   , coalesce(RTRIM(INVCYCL), '')                           AS INVCYCL              ")
        ...
        SQLStr.AppendLine("   , coalesce(RTRIM(INVSUBCD), '')                          AS INVSUBCD             ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYKEIJYOBRANCHCD), '')                 AS PAYKEIJYOBRANCHCD    ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYFILINGBRANCH), '')                   AS PAYFILINGBRANCH      ")
        SQLStr.AppendLine("   , coalesce(RTRIM(TAXCALCUNIT), '')                       AS TAXCALCUNIT          ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYKESAIKBN), '')                       AS PAYKESAIKBN          ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYBANKCD), '')                         AS PAYBANKCD            ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYBANKBRANCHCD), '')                   AS PAYBANKBRANCHCD      ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYACCOUNTTYPE), '')                    AS PAYACCOUNTTYPE       ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYACCOUNTNO), '')                      AS PAYACCOUNTNO         ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYACCOUNTNM), '')                      AS PAYACCOUNTNM         ")
        SQLStr.AppendLine("   , coalesce(RTRIM(PAYTEKIYO), '')                         AS PAYTEKIYO            ")
        ...
        SQLStr.AppendLine("     LNG.LNT0114_REKEJHIST                                                                            ")
        ...
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNM0003H SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNM0003H Select"
    ...
    Protected Sub MODIFYYMGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLStr.AppendLine(" FROM lng.LNT0114_REKEJHIST ")
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0114_REKEJHIST SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:LNT0114_REKEJHIST Select"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                             'ログ出力
...
#Region "ﾀﾞｳﾝﾛｰﾄﾞ"
    ...
    Protected Sub WF_EXCELPDF(ByVal WW_FILETYPE As Integer)
        ...
        '最大列(RANGE)を取得
        Dim WW_MAXCOL As Integer = [Enum].GetValues(GetType(LNM0003WRKINC.HISTORYEXCELCOL)).Cast(Of Integer)().Max()
        ...
        wb.ActiveSheet.Range("C1").Value = "コンテナ取引先マスタ変更履歴一覧"
    ...
    Public Sub SetHEADER(ByVal sheet As IWorksheet, ByVal WW_MAXCOL As Integer)
        ...
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = "操作区分"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = "変更区分"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = "変更日時"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.MODIFYUSER).Value = "変更USER"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.DELFLG).Value = "削除フラグ"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = "発駅コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = "発受託人コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBCD).Value = "発受託人サブコード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEENM).Value = "発受託人名称"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBNM).Value = "発受託人サブ名称"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBKANA).Value = "発受託人名称（カナ）"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.TORICODE).Value = "取引先コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.ELIGIBLEINVOICENUMBER).Value = "適格請求書登録番号"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.INVKEIJYOBRANCHCD).Value = "請求項目 計上店コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.INVCYCL).Value = "請求項目 請求サイクル"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.INVFILINGDEPT).Value = "請求項目 請求書提出部店"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.INVKESAIKBN).Value = "請求項目 請求書決済区分"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.INVSUBCD).Value = "請求項目 請求書細分コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYKEIJYOBRANCHCD).Value = "支払項目 費用計上店コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYFILINGBRANCH).Value = "支払項目 支払書提出支店"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.TAXCALCUNIT).Value = "支払項目 消費税計算単位"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYKESAIKBN).Value = "支払項目 決済区分"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYBANKCD).Value = "支払項目 銀行コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYBANKBRANCHCD).Value = "支払項目 銀行支店コード"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYACCOUNTTYPE).Value = "支払項目 口座種別"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYACCOUNTNO).Value = "支払項目 口座番号"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYACCOUNTNM).Value = "支払項目 口座名義人"
        sheet.Cells(WW_HEADERROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYTEKIYO).Value = "支払項目 支払摘要"
    ...
    Public Sub SetDETAIL(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        For Each Row As DataRow In LNM0003tbl.Rows
            '値
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.OPERATEKBNNAME).Value = Row("OPERATEKBNNAME") '操作区分
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.MODIFYKBNNAME).Value = Row("MODIFYKBNNAME") '変更区分
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.MODIFYYMD).Value = Row("MODIFYYMD") '変更日時
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.MODIFYUSER).Value = Row("MODIFYUSER") '変更USER
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.DELFLG).Value = Row("DELFLG") '削除フラグ
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPSTATION).Value = Row("DEPSTATION") '発駅コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEECD).Value = Row("DEPTRUSTEECD") '発受託人コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBCD).Value = Row("DEPTRUSTEESUBCD") '発受託人サブコード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEENM).Value = Row("DEPTRUSTEENM") '発受託人名称
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBNM).Value = Row("DEPTRUSTEESUBNM") '発受託人サブ名称
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.DEPTRUSTEESUBKANA).Value = Row("DEPTRUSTEESUBKANA") '発受託人名称（カナ）
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.TORICODE).Value = Row("TORICODE") '取引先コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.ELIGIBLEINVOICENUMBER).Value = Row("ELIGIBLEINVOICENUMBER") '適格請求書登録番号
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.INVKEIJYOBRANCHCD).Value = Row("INVKEIJYOBRANCHCD") '請求項目 計上店コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.INVCYCL).Value = Row("INVCYCL") '請求項目 請求サイクル
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.INVFILINGDEPT).Value = Row("INVFILINGDEPT") '請求項目 請求書提出部店
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.INVKESAIKBN).Value = Row("INVKESAIKBN") '請求項目 請求書決済区分
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.INVSUBCD).Value = Row("INVSUBCD") '請求項目 請求書細分コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYKEIJYOBRANCHCD).Value = Row("PAYKEIJYOBRANCHCD") '支払項目 費用計上店コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYFILINGBRANCH).Value = Row("PAYFILINGBRANCH") '支払項目 支払書提出支店
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.TAXCALCUNIT).Value = Row("TAXCALCUNIT") '支払項目 消費税計算単位
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYKESAIKBN).Value = Row("PAYKESAIKBN") '支払項目 決済区分
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYBANKCD).Value = Row("PAYBANKCD") '支払項目 銀行コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYBANKBRANCHCD).Value = Row("PAYBANKBRANCHCD") '支払項目 銀行支店コード
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYACCOUNTTYPE).Value = Row("PAYACCOUNTTYPE") '支払項目 口座種別
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYACCOUNTNO).Value = Row("PAYACCOUNTNO") '支払項目 口座番号
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYACCOUNTNM).Value = Row("PAYACCOUNTNM") '支払項目 口座名義人
            sheet.Cells(WW_ACTIVEROW, LNM0003WRKINC.HISTORYEXCELCOL.PAYTEKIYO).Value = Row("PAYTEKIYO") '支払項目 支払摘要

            '変更区分が変更後の行の場合
            If Row("MODIFYKBN") = LNM0003WRKINC.MODIFYKBN.AFTDATA Then
                '変更箇所を塗りつぶし
    ...
    Public Sub SetMODIFYHATCHING(ByVal sheet As IWorksheet, ByRef WW_ACTIVEROW As Integer)
        ...
        '変更チェック開始列を取得
        Dim WW_STCOL As Integer = LNM0003WRKINC.HISTORYEXCELCOL.DELFLG   '削除フラグ
