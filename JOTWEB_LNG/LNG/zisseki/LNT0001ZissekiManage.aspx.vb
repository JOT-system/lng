            ...
            If IsPostBack Then
                ...
                If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                    ...
                    Select Case WF_ButtonClick.Value
                        ...
                        Case "WF_ButtonKintone"                    '実績取込（アボカド）ボタンクリック
                            WF_KintoneGetRecodes()
...
#Region "初期化処理"
    ...
    Protected Sub Initialize()
        ...
        '○画面ID設定
        Master.MAPID = LNT0001WRKINC.MAPID
...
#Region "画面表示設定処理"
    ...
    Protected Sub WW_ScreenEnabledSet(Optional ByVal researchFlg As Boolean = True)
        ...
        Next
        '一覧(再)表示
        gvLNT0001.DataSource = New DataView(dt) With {
    ...
    ''' <param name="e"></param>
    Protected Sub gvLNT0001_PreRender(sender As Object, e As EventArgs)
        'GridViewオブジェクト取得
    ...
    ''' 検査コンテナ一覧表(DataBound)
    ''' </summary>
    Protected Sub gvLNT0001_DataBound(sender As Object, e As EventArgs)
    ...
    Protected Overrides Sub Render(writer As HtmlTextWriter)
        ...
        Dim idPos = editedPageSource.IndexOf("contents1_gvLNT0001")
        If idPos < 0 Then
...
#Region "データ取得処理"
    ...
    Protected Sub MapDataGet(ByVal SQLcon As MySqlConnection)
        ...
        SQLBldr.AppendLine("     , '' AS CTNTYPE")
        SQLBldr.AppendLine("     , '' AS CTNNO")
        SQLBldr.AppendLine("     , '' AS  CONTNUM")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m') AS CONTRUCTIONYM")
        SQLBldr.AppendLine("     , '' AS  ORGCODE")
        SQLBldr.AppendLine("     , '' AS  ARRSTATION")
        SQLBldr.AppendLine("     , '' AS  ARRSTATIONNAME")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS  TRAINSBEFORERUNYMD")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS  TRAINSNEXTRUNYMD")
        SQLBldr.AppendLine("     , '' AS YEAR4_AFTER")
        SQLBldr.AppendLine("     , '' AS  YEAR4FLG")
        SQLBldr.AppendLine("     , '' AS  YEAR4NOREGFLG")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS YEAR4_INSPECTYMD")
        ...
        SQLBldr.AppendLine("     , '' AS YEAR8_AFTER")
        SQLBldr.AppendLine("     , '' AS  YEAR8FLG")
        SQLBldr.AppendLine("     , '' AS  YEAR8NOREGFLG")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS YEAR8_INSPECTYMD")
        ...
        SQLBldr.AppendLine("     , '' AS YEAR12_AFTER")
        SQLBldr.AppendLine("     , '' AS  YEAR12FLG")
        SQLBldr.AppendLine("     , '' AS  YEAR12NOREGFLG")
        SQLBldr.AppendLine("     , 0 AS YEAR12_NOREGISTFLG")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS YEAR12_INSPECTYMD")
        ...
        SQLBldr.AppendLine("     , '' AS YEAR16_AFTER")
        SQLBldr.AppendLine("     ,'' AS  YEAR16OVERFLG")
        SQLBldr.AppendLine("     , 0 AS YEARN_SEQ")
        SQLBldr.AppendLine("     , 0 AS YEARN_YEAR")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS YEARN_INSPECTYMD")
        ...
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS ADD_INSPECTYMD")
        ...
        SQLBldr.AppendLine("     , '' AS CTNTYPE")
        SQLBldr.AppendLine("     , '' AS CTNNO")
        SQLBldr.AppendLine("     , '' AS INSPELNTYPE")
        SQLBldr.AppendLine("     , '' AS INSPECTSEQ")
        SQLBldr.AppendLine("     , '' AS INSPECTYEAR")
        SQLBldr.AppendLine("     , date_format(CURDATE(), '%Y/%m/%d') AS INSPECTYMD")
        SQLBldr.AppendLine("     , '' AS INSPECTCODE")
        SQLBldr.AppendLine("     , '' AS INSPECTNAME")
        SQLBldr.AppendLine("     , '' AS INSPECTVENDOR")
        SQLBldr.AppendLine("     , '' AS INSPECTVENDORNAME")
        SQLBldr.AppendLine("     , '' AS ENFORCEPLACE")
        'SQLBldr.AppendLine(" FROM")
        ...
        SQLBldr.Clear()
        SQLBldr.AppendLine(" SELECT")
        SQLBldr.AppendLine("     '' AS 'code'")
        SQLBldr.AppendLine("     , '' AS 'name'")
        'SQLBldr.AppendLine(" FROM")
    ...
    Private Sub GetStationName()
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0001C GetStationName")
            CS0011LOGWrite.INFSUBCLASS = "LNT0001C"         'SUBクラス名
    ...
    Private Function GetInspectCodeTable() As DataTable
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0001C GetInspectCodeTable")
            CS0011LOGWrite.INFSUBCLASS = "LNT0001C"         'SUBクラス名
    ...
    Private Function GetInspectVendorTable() As DataTable
        ...
        Try
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "LNT0001C GetInspectVendorTable")
            CS0011LOGWrite.INFSUBCLASS = "LNT0001C"         'SUBクラス名
...
#Region "データ更新処理"
    ...
    Private Function WF_InspectUpdateByFile() As Boolean
        ...
        Try
            ...
            If Not "ID:LNT0001C".Equals(sheet.Range("A1").Value) Then
                'エラーメッセージ出力
                Master.Output(C_MESSAGE_NO.CTN_UNIVERSAL_MESSAGE, C_MESSAGE_TYPE.ERR,
            ...
            Try
                Using SQLcmd As New MySqlCommand(UpdSQLBldr.ToString, SQLcon)
                    ...
                    With SQLcmd.Parameters
                        ...
                        .Add("@UPDPGID", MySqlDbType.VarChar, 40).Value = "LNT0001ZissekiManage"
                    End With
...
#Region "アボカドシステム（Kintone）レコード取得"
    ...
    ''' </summary>
    Private Sub WF_KintoneGetRecodes()


        Try
            'アップロードAPIの実行
            'WebAPI実行クラスの宣言
            Dim ApiBaseUrl As String = "https://jot.cybozu.com/k/v1/records.json"
            Dim ApiBasicPass As String = "Basic am90dXNlcjppc3VzZXI="
            Dim ApiToken As String = "1g1vJZ8agOyof2WEuKKqDTQmVF3h9EXeLT9YJkrN"
            Dim ApiApplID As String = "1007"

            Dim kintoneApi As New CS0054KintoneApi(ApiBaseUrl, ApiBasicPass, ApiToken, ApiApplID)
            Dim retTbl = kintoneApi.GetRecords()
            If retTbl.Rows.Count > 0 Then
                '取得したKintoneデータ（データテーブル）を処理する
            End If
