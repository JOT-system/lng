...
''' 発駅・通運別合計表帳票作成クラス
''' </summary>
Public Class LNT0012_TransportTotalReport_DIODOC
    ...
    Public Sub New(mapId As String, excelFileName As String, printDataClass As DataTable)
        Try
            ...
            For i As Integer = 0 To WW_Workbook.Worksheets.Count - 1
                If WW_Workbook.Worksheets(i).Name = "発駅・通運別合計表" Then
                    WW_SheetNo = i
    ...
    Public Function CreateExcelPrintData(Trusteecd As Integer, Sort As String, addsub As String) As String
        Dim ReportName As String = "発駅・通運別合計表_"
        ...
        Try
            ...
            Dim Table7(5, 32, 3) As Long
            Dim Table8(5, 32, 3) As Long
            Dim Table9(5, 32, 3) As Long
            ...
            Dim WB_JRFixedFare As Long = 0
            Dim WB_NittuFreesend As Long = 0
            Dim WB_OwnDiscountFee As Long = 0
            Dim WB_ReturnFare As Long = 0
            ...
            For Each row As DataRow In PrintData.Rows
                ...
                If fstflg = "0" Then
                    ...
                    '明細ヘッダー情報(数量加算)
                    EditAddArea(row, WB_Quantity, WB_UseFee, WB_JRFixedFare, WB_NittuFreesend,
                                WB_OwnDiscountFee, WB_ReturnFare, WB_ShipBurdenFee, WB_ManageFee, addsub)
                    '明細行表示内容計算(テーブル加算)
                    EditCalcArea(row, Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, addsub)
                    fstflg = "1"
                    lastRow = row
                    Continue For
                End If
                ...
                If row_cnt = PrintData.Rows.Count Then
                    'EditAddArea(row, WB_Quantity, WB_UseFee, WB_JRFixedFare, WB_NittuFreesend, WB_OwnDiscountFee, WB_ReturnFare, WB_ShipBurdenFee, WB_ManageFee)
                    'EditCalcArea(row, Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, addsub)
                    lastRow = row
                    'Exit For
                End If

                If Sort <> "2" Then      '発受託人、発駅順
                    '読み飛ばし区分の初期化
                    SkipFLG = "0"

                    '取引先名が一つ前のレコードと別の場合
                    If row("TORINAME").ToString <> WW_ToriName Then
                        SkipFLG = "1"
                    End If
                    ...
                    If SkipFLG = "0" Then
                        ...
                        If Key_Trustee = CType(row("DEPTRUSTEECD"), Integer) AndAlso Key_TrusteeSub = CType(row("DEPTRUSTEESUBCD"), Integer) AndAlso Key_Station = CType(row("DEPSTATIONCD"), Integer) AndAlso CType(row("RECODETYPE"), Integer) <> 2 Then
                            '明細ヘッダー情報(数量加算)
                            EditAddArea(row, WB_Quantity, WB_UseFee, WB_JRFixedFare, WB_NittuFreesend, WB_OwnDiscountFee, WB_ReturnFare, WB_ShipBurdenFee, WB_ManageFee, addsub)
                            '明細行表示内容計算(テーブル加算)
                            EditCalcArea(row, Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, addsub)
                            lastRow = row
                            Continue For
                        ElseIf Key_Trustee = CType(row("DEPTRUSTEECD"), Integer) AndAlso Key_TrusteeSub = CType(row("DEPTRUSTEESUBCD"), Integer) AndAlso Key_Station = CType(row("DEPSTATIONCD"), Integer) AndAlso CType(row("RECODETYPE"), Integer) = 2 Then
                            ...
                            If lastRow("RECODETYPE").ToString <> row("RECODETYPE").ToString Then
                                ...
                                EditDetailArea(idx, row, PageNum, Sort, Key_Orgcode, Key_Toricode, Key_Trustee, Key_TrusteeSub, Key_Station, WB_OrgName, WB_ToriName, WB_TrusteeName, WB_TrusteeSubName, WB_StationName,
                                               WB_InvKeijoBranch, WB_InvFilingDepT, WB_Quantity, WB_UseFee, WB_JRFixedFare, WB_NittuFreesend, WB_OwnDiscountFee,
                                               WB_ReturnFare, WB_ShipBurdenFee, WB_ManageFee)
                                '明細行出力
                                EditTotalAreaA(idx, row, lastRow, PageNum, Key_Orgcode, Key_Toricode, Key_Trustee, Key_TrusteeSub, Key_Station, WW_TrusteeName, WW_TrusteeSubName, WW_StationName,
                                               Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, Sort, "1", "1")
                            End If
                            '追加明細の中で請求年月の変更がある場合
                            If lastRow("KEIJOYM").ToString <> row("KEIJOYM").ToString Then
                                '明細行出力
                                EditTotalAreaA(idx, row, lastRow, PageNum, Key_Orgcode, Key_Toricode, Key_Trustee, Key_TrusteeSub, Key_Station, WW_TrusteeName, WW_TrusteeSubName, WW_StationName,
                                               Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, Sort, "1", "2")
                            End If
                            '明細ヘッダー情報(数量加算)
                            EditAddArea(row, WB_Quantity, WB_UseFee, WB_JRFixedFare, WB_NittuFreesend, WB_OwnDiscountFee, WB_ReturnFare, WB_ShipBurdenFee, WB_ManageFee, addsub)
                            '明細行表示内容計算(テーブル加算)
                        ...
                        '明細行出力
                        EditTotalAreaA(idx, row, lastRow, PageNum, Key_Orgcode, Key_Toricode, Key_Trustee, Key_TrusteeSub, Key_Station, WW_TrusteeName, WW_TrusteeSubName, WW_StationName,
                                               Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, Sort, "1", "3")
                        '明細ヘッダー情報(数量加算)
                        EditAddArea(row, WB_Quantity, WB_UseFee, WB_JRFixedFare, WB_NittuFreesend, WB_OwnDiscountFee, WB_ReturnFare, WB_ShipBurdenFee, WB_ManageFee, addsub)
                        '明細行表示内容計算(テーブル加算)
                        EditCalcArea(row, Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, addsub)
                    ...
                    If Table2(1, 31, 1) <> Table2(2, 31, 1) OrElse Table2(1, 31, 1) <> Table2(3, 31, 1) Then
                        '明細行出力
                        EditTotalAreaA(idx, row, lastRow, PageNum, Key_Orgcode, Key_Toricode, Key_Trustee, Key_TrusteeSub, Key_Station, WW_TrusteeName, WW_TrusteeSubName, WW_StationName,
                                               Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, Sort, "0", "3")
                    Else
                        'データの初期化
                        For j As Integer = 1 To 32
                            For k As Integer = 1 To 3
                    ...
                    EditTotalAreaB(idx, row, PageNum, Key_Orgcode, Key_Toricode, Key_Trustee, Key_TrusteeSub, Key_Station, WW_OrgName, WW_ToriName, WW_TrusteeName, WW_TrusteeSubName, WW_StationName, WW_PartnerCamp,
                                   Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, Sort)
            ...
            EditTotalAreaD(idx, lastRow, PageNum, Key_Orgcode, Key_Orgcode_Total, Key_Toricode, Key_Trustee, Key_TrusteeSub, Key_Station, WW_OrgName, WW_OrgName_Total, WW_ToriName, WW_TrusteeName, WW_TrusteeSubName, WW_StationName, WW_PartnerCamp,
                                       Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, Sort)
            '合計C
            EditTotalAreaC(idx, lastRow, PageNum, Table1, Table2, Table3, Table4, Table5, Table6, Table7, Table8, Table9, Sort)
    ...
    Private Sub EditHeaderArea(
        ...
        Try
            ...
            ElseIf sort = "2" Then
                srcRange = WW_Workbook.Worksheets(WW_tmpSheetNo).Range("A8:S11")
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString())
                srcRange.Copy(destRange)
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range(String.Format("{0}:{0}", idx))
            End If
            ...
            '〇頁数
            WW_Workbook.Worksheets(WW_SheetNo).Range("Q" + idx.ToString()).Value = pageNum
            '◯処理日
            WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = DateTime.Now.ToShortDateString
            '◯処理時刻
            WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = DateTime.Now.ToShortTimeString
            '〇締め日
            WW_Workbook.Worksheets(WW_SheetNo).Range("D" + (idx + 1).ToString()).Value = row("HEADER_A")
            '〇ベース
            WW_Workbook.Worksheets(WW_SheetNo).Range("J" + (idx + 1).ToString()).Value = row("HEADER_B")
            '〇請求先
            WW_Workbook.Worksheets(WW_SheetNo).Range("N" + (idx + 1).ToString()).Value = row("HEADER_C")
            '〇加減額
            WW_Workbook.Worksheets(WW_SheetNo).Range("Q" + (idx + 1).ToString()).Value = row("HEADER_D")
            '〇支店
    ...
    Private Sub EditDetailArea(
         ...
         ByRef WB_JRFixedFare As Long,
         ByRef WB_NittuFreesend As Long,
         ByRef WB_OwnDiscountFee As Long,
         ByRef WB_ReturnFare As Long,
        ...
        '明細行コピー
        srcRange = WW_Workbook.Worksheets(WW_tmpSheetNo).Range("A14:S14")
        destRange = WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString())
        srcRange.Copy(destRange)
        destRange = WW_Workbook.Worksheets(WW_SheetNo).Range(String.Format("{0}:{0}", idx))
        ...
        If Sort <> "2" Then
            ...
            '請求支店
            WW_Workbook.Worksheets(WW_SheetNo).Range("O" + idx.ToString()).Value = Key_Orgcode
            '請求先コード
            WW_Workbook.Worksheets(WW_SheetNo).Range("P" + idx.ToString()).Value = Key_Toricode
            '発受託人コード
            WW_Workbook.Worksheets(WW_SheetNo).Range("Q" + idx.ToString()).Value = Key_Trustee
            '発受託人サブコード
            WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = Key_TrusteeSub
            '発駅コード
            WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = Key_Station
        ElseIf Sort = "2" Then
            '請求支店
            WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString()).Value = WB_OrgName
            '請求先
            ...
            WW_Workbook.Worksheets(WW_SheetNo).Range("P" + idx.ToString()).Value = Key_Toricode
            '発駅コード
            WW_Workbook.Worksheets(WW_SheetNo).Range("Q" + idx.ToString()).Value = Key_Station
            '発受託人コード
            WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = Key_Trustee
            '発受託人サブコード
            WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = Key_TrusteeSub
        End If
        '店 収請
        ...
        'JR運賃
        WW_Workbook.Worksheets(WW_SheetNo).Range("L" + idx.ToString()).Value = WB_JRFixedFare
        '私割引
        WW_Workbook.Worksheets(WW_SheetNo).Range("M" + idx.ToString()).Value = WB_OwnDiscountFee
        '割戻
        WW_Workbook.Worksheets(WW_SheetNo).Range("N" + idx.ToString()).Value = WB_ReturnFare
    ...
    Private Sub EditTotalAreaA(
         ...
         ByRef Table7(,,) As Long,
         ByRef Table8(,,) As Long,
         ByRef Table9(,,) As Long,
         ByVal Sort As String,
        ...
        While 0 = 0
            ...
            'KAP01.
            SkipFLG = "0"
            If Table2(1, AA, BB) = 0 AndAlso Table6(1, AA, BB) = 0 AndAlso Table8(1, AA, BB) = 0 Then
                SkipFLG = "1"
            End If
            ...
            If SkipFLG = "0" Then
                ...
                '合計行Aコピー
                srcRange = WW_Workbook.Worksheets(WW_tmpSheetNo).Range("A17:S17")
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString())
                srcRange.Copy(destRange)
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range(String.Format("{0}:{0}", idx))
                ...
                End If
                If AA = 31 Then
                    If Table9(1, 31, 1) = Table9(1, 32, 3) Then
                        EndFLG = "1"
                    End If
                End If
                ...
                If EndFLG = "0" Then
                    ...
                    'その他収入
                    WW_Workbook.Worksheets(WW_SheetNo).Range("I" + idx.ToString()).Value = Table8(1, AA, BB)
                    '通運負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("J" + idx.ToString()).Value = Table6(1, AA, BB)
                    '荷主負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("K" + idx.ToString()).Value = Table7(1, AA, BB)
                    'JR運賃
                    WW_Workbook.Worksheets(WW_SheetNo).Range("L" + idx.ToString()).Value = Table3(1, AA, BB)
                    '私割引
                    WW_Workbook.Worksheets(WW_SheetNo).Range("M" + idx.ToString()).Value = Table4(1, AA, BB)
                    '割戻
                    WW_Workbook.Worksheets(WW_SheetNo).Range("N" + idx.ToString()).Value = Table5(1, AA, BB)
                    If AA = 32 Then
                        '請求額
                        WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = "請求額"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).NumberFormat = "#,##0"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = Table9(1, AA, BB)
                    End If
                    idx += 1
                End If
            End If
    ...
    Private Sub EditTotalAreaB(
        ...
        While 0 = 0
            ...
            'KAP01.
            SkipFLG = "0"
            If Table2(2, AA, BB) = 0 AndAlso Table6(2, AA, BB) = 0 AndAlso Table8(2, AA, BB) = 0 Then
                SkipFLG = "1"
            End If
            ...
            If SkipFLG = "0" Then
                ...
                '合計行Bコピー
                srcRange = WW_Workbook.Worksheets(WW_tmpSheetNo).Range("A20:S20")
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString())
                srcRange.Copy(destRange)
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range(String.Format("{0}:{0}", idx))
                ...
                EndFLG = "0"
                If AA = 31 Then
                    If Table9(2, 31, 1) = Table9(2, 32, 3) Then
                        EndFLG = "1"
                    End If
                End If
                ...
                If EndFLG = "0" Then
                    ...
                    'その他収入
                    WW_Workbook.Worksheets(WW_SheetNo).Range("I" + idx.ToString()).Value = Table8(2, AA, BB)
                    '通運負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("J" + idx.ToString()).Value = Table6(2, AA, BB)
                    '荷主負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("K" + idx.ToString()).Value = Table7(2, AA, BB)
                    'JR運賃
                    WW_Workbook.Worksheets(WW_SheetNo).Range("L" + idx.ToString()).Value = Table3(2, AA, BB)
                    '私割引
                    WW_Workbook.Worksheets(WW_SheetNo).Range("M" + idx.ToString()).Value = Table4(2, AA, BB)
                    '割戻
                    WW_Workbook.Worksheets(WW_SheetNo).Range("N" + idx.ToString()).Value = Table5(2, AA, BB)
                    If AA = 32 Then
                        '請求額
                        WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = "請求額"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).NumberFormat = "#,##0"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = Table9(2, AA, BB)
                    End If
                    idx += 1
                End If
            End If
    ...
    Private Sub EditTotalAreaC(
        ...
        While 0 = 0
            ...
            'KAP01.
            SkipFLG = "0"
            If Table2(3, AA, BB) = 0 AndAlso Table6(3, AA, BB) = 0 AndAlso Table8(3, AA, BB) = 0 Then
                SkipFLG = "1"
            End If
            ...
            If SkipFLG = "0" Then
                ...
                '合計行Cコピー
                srcRange = WW_Workbook.Worksheets(WW_tmpSheetNo).Range("A23:S23")
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString())
                srcRange.Copy(destRange)
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range(String.Format("{0}:{0}", idx))
                ...
                EndFLG = "0"
                If AA = 31 Then
                    If Table9(3, 31, 1) = Table9(3, 32, 3) Then
                        EndFLG = "1"
                    End If
                End If
                ...
                If EndFLG = "0" Then
                    ...
                    'その他収入
                    WW_Workbook.Worksheets(WW_SheetNo).Range("I" + idx.ToString()).Value = Table8(3, AA, BB)
                    '通運負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("J" + idx.ToString()).Value = Table6(3, AA, BB)
                    '荷主負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("K" + idx.ToString()).Value = Table7(3, AA, BB)
                    'JR運賃
                    WW_Workbook.Worksheets(WW_SheetNo).Range("L" + idx.ToString()).Value = Table3(3, AA, BB)
                    '私割引
                    WW_Workbook.Worksheets(WW_SheetNo).Range("M" + idx.ToString()).Value = Table4(3, AA, BB)
                    '割戻
                    WW_Workbook.Worksheets(WW_SheetNo).Range("N" + idx.ToString()).Value = Table5(3, AA, BB)
                    If AA = 32 Then
                        '請求額
                        WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = "請求額"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).NumberFormat = "#,##0"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = Table9(3, AA, BB)
                    End If
                    idx += 1
                End If
            End If
    ...
    Private Sub EditTotalAreaD(
        ...
        While 0 = 0
            ...
            'KAP01.
            SkipFLG = "0"
            If Table2(4, AA, BB) = 0 AndAlso Table6(4, AA, BB) = 0 AndAlso Table8(4, AA, BB) = 0 Then
                SkipFLG = "1"
            End If
            ...
            If SkipFLG = "0" Then
                ...
                '合計行Bコピー
                srcRange = WW_Workbook.Worksheets(WW_tmpSheetNo).Range("A26:S26")
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString())
                srcRange.Copy(destRange)
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range(String.Format("{0}:{0}", idx))
                ...
                EndFLG = "0"
                If AA = 31 Then
                    If Table9(4, 31, 1) = Table9(4, 32, 3) Then
                        EndFLG = "1"
                    End If
                End If
                ...
                If EndFLG = "0" Then
                    ...
                    'その他収入
                    WW_Workbook.Worksheets(WW_SheetNo).Range("I" + idx.ToString()).Value = Table8(4, AA, BB)
                    '通運負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("J" + idx.ToString()).Value = Table6(4, AA, BB)
                    '荷主負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("K" + idx.ToString()).Value = Table7(4, AA, BB)
                    'JR運賃
                    WW_Workbook.Worksheets(WW_SheetNo).Range("L" + idx.ToString()).Value = Table3(4, AA, BB)
                    '私割引
                    WW_Workbook.Worksheets(WW_SheetNo).Range("M" + idx.ToString()).Value = Table4(4, AA, BB)
                    '割戻
                    WW_Workbook.Worksheets(WW_SheetNo).Range("N" + idx.ToString()).Value = Table5(4, AA, BB)
                    If AA = 32 Then
                        '請求額
                        WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = "請求額"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).NumberFormat = "#,##0"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = Table9(4, AA, BB)
                    End If
                    idx += 1
                End If
            End If
    ...
    Private Sub EditTotalAreaE(
        ...
        While 0 = 0
            ...
            'KAP01.
            SkipFLG = "0"
            If Table2(5, AA, BB) = 0 AndAlso Table6(5, AA, BB) = 0 AndAlso Table8(5, AA, BB) = 0 Then
                SkipFLG = "1"
            End If
            ...
            If SkipFLG = "0" Then
                ...
                '合計行Bコピー
                srcRange = WW_Workbook.Worksheets(WW_tmpSheetNo).Range("A29:S29")
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range("A" + idx.ToString())
                srcRange.Copy(destRange)
                destRange = WW_Workbook.Worksheets(WW_SheetNo).Range(String.Format("{0}:{0}", idx))
                ...
                EndFLG = "0"
                If AA = 31 Then
                    If Table9(5, 31, 1) = Table9(5, 32, 3) Then
                        EndFLG = "1"
                    End If
                End If
                ...
                If EndFLG = "0" Then
                    ...
                    'その他収入
                    WW_Workbook.Worksheets(WW_SheetNo).Range("I" + idx.ToString()).Value = Table8(5, AA, BB)
                    '通運負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("J" + idx.ToString()).Value = Table6(5, AA, BB)
                    '荷主負担
                    WW_Workbook.Worksheets(WW_SheetNo).Range("K" + idx.ToString()).Value = Table7(5, AA, BB)
                    'JR運賃
                    WW_Workbook.Worksheets(WW_SheetNo).Range("L" + idx.ToString()).Value = Table3(5, AA, BB)
                    '私割引
                    WW_Workbook.Worksheets(WW_SheetNo).Range("M" + idx.ToString()).Value = Table4(5, AA, BB)
                    '割戻
                    WW_Workbook.Worksheets(WW_SheetNo).Range("N" + idx.ToString()).Value = Table5(5, AA, BB)
                    If AA = 32 Then
                        '請求額
                        WW_Workbook.Worksheets(WW_SheetNo).Range("R" + idx.ToString()).Value = "請求額"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).NumberFormat = "#,##0"
                        WW_Workbook.Worksheets(WW_SheetNo).Range("S" + idx.ToString()).Value = Table9(5, AA, BB)
                    End If
                    idx += 1
                End If
            End If
    ...
    Private Sub EditAddArea(
        ...
        If addsub = "1" Then
            If row("FLG").ToString <> "9" AndAlso row("FLG").ToString <> "8" Then
                ...
                JRFixedFare += CType(row("JRFIXEDFARE"), Long)
                NittuFreesend += CType(row("NITTSUFREESEND"), Long)
                OwnDiscountFee += CType(row("OWNDISCOUNTFEE"), Long)
                ReturnFare += CType(row("RETURNFARE"), Long)
    ...
    Private Sub EditCalcArea(
        ...
        While 0 = 0
            ...
            Table3(AA, BB, CC) += CType(row("JRFIXEDFARE"), Long)
            Table4(AA, BB, CC) += CType(row("OWNDISCOUNTFEE"), Long)
            Table5(AA, BB, CC) += CType(row("RETURNFARE"), Long)
            Table6(AA, BB, CC) += CType(row("NITTSUFREESEND"), Long)
            Table7(AA, BB, CC) += CType(row("SHIPBURDENFEE"), Long)
            Table8(AA, BB, CC) += CType(row("TOTAL"), Long)
            If AA < 5 Then
                AA += 1
                Continue While
            End If
        ...
        While 0 = 0
            ...
            Table3(AA, 31, CC) += CType(row("JRFIXEDFARE"), Long)
            Table4(AA, 31, CC) += CType(row("OWNDISCOUNTFEE"), Long)
            Table5(AA, 31, CC) += CType(row("RETURNFARE"), Long)
            Table6(AA, 31, CC) += CType(row("NITTSUFREESEND"), Long)
            Table7(AA, 31, CC) += CType(row("SHIPBURDENFEE"), Long)
            Table8(AA, 31, CC) += CType(row("TOTAL"), Long)

            Table9(AA, 31, CC) += CType(row("USEFEE"), Long)
            Table9(AA, 31, CC) += CType(row("NITTSUFREESEND"), Long)
            Table9(AA, 31, CC) += CType(row("SHIPBURDENFEE"), Long)
            Table9(AA, 31, CC) += CType(row("TOTAL"), Long)

            Table1(AA, 32, 3) += CType(row("QUANTITY"), Long)
            Table2(AA, 32, 3) += CType(row("USEFEE"), Long)
            Table3(AA, 32, 3) += CType(row("JRFIXEDFARE"), Long)
            Table4(AA, 32, 3) += CType(row("OWNDISCOUNTFEE"), Long)
            Table5(AA, 32, 3) += CType(row("RETURNFARE"), Long)
            Table6(AA, 32, 3) += CType(row("NITTSUFREESEND"), Long)
            Table7(AA, 32, 3) += CType(row("SHIPBURDENFEE"), Long)
            Table8(AA, 32, 3) += CType(row("TOTAL"), Long)

            Table9(AA, 32, 3) += CType(row("USEFEE"), Long)
            Table9(AA, 32, 3) += CType(row("NITTSUFREESEND"), Long)
            Table9(AA, 32, 3) += CType(row("SHIPBURDENFEE"), Long)
            Table9(AA, 32, 3) += CType(row("TOTAL"), Long)
